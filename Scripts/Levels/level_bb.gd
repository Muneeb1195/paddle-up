...
func create_block_field() -> void:
	new_block_pos = GameConfig.BLOCK_START_POS
	for rows : int in GameConfig.STARTING_ROWS:
		_create_block_row()
		new_block_pos.x = GameConfig.BLOCK_START_POS.x
		new_block_pos.y += GameConfig.GRID_SIZE
...
func _move_old_blocks() -> Tween:
	...
	for old_bricks : StaticBody2D in blocks_node.get_children():
		tween.tween_property(old_bricks, "position:y", old_bricks.position.y + GameConfig.GRID_SIZE, 0.15)
