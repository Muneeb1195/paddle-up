...
@onready var ball: BallPong = $Ball
...
var cpu_points : int:
	...
	set(value):
		...
		_flash_score(true)
		if _cpu_points >= GameConfig.MAX_POINTS:
...
var player_points : int :
	...
	set(value):
		...
		_flash_score(false)
		if _player_points >= GameConfig.MAX_POINTS:
...
func _set_difficulty_state() -> void:
	...
	match difficulty:
		DIFFICULTY.Easy:
			ball_starting_speed = GameConfig.STARTING_SPEEDS[0]
			_base_speed_mod = GameConfig.BASE_SPEED_MODS[0]
		DIFFICULTY.Medium:
			ball_starting_speed = GameConfig.STARTING_SPEEDS[1]
			_base_speed_mod = GameConfig.BASE_SPEED_MODS[1]
		DIFFICULTY.Hard:
			ball_starting_speed = GameConfig.STARTING_SPEEDS[2]
			_base_speed_mod = GameConfig.BASE_SPEED_MODS[2]
...
func _start_countdown() -> void:
	...
	await get_tree().create_timer(GameConfig.COUNTDOWN_SEC, false).timeout
...
func _increase_player_point() -> void:
	...
	player_points += GameConfig.SCORE_INCREMENT
	_move_ball_back()

func _increase_cpu_point() -> void:
	_on_goal_scored(true)
	cpu_points += GameConfig.SCORE_INCREMENT
	_move_ball_back()
