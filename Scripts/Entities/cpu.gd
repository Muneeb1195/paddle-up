...
func _process(delta: float) -> void:
	...
	y_dist = abs(ball.position.y - position.y)
...
func trigger_reaction_delay() -> void:
	_has_pending_target = true
	_delay_timer = 0.0

func _move_back(delta : float) -> void:
	direction = position.direction_to(GameConfig.PADDLE_HOME)
	if position.distance_to(GameConfig.PADDLE_HOME) > 10:
		_move(delta)
	else:
		_stop(delta)
